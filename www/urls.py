...
def make_signed_cookie(id, passwd, max_age):
    expires = str(int(time.time() + (max_age or 86400)))
    L = [id, expires, hashlib.md5('%s-%s-%s-%s' % (id, passwd, expires, _COOKIE_KEY)).hexdugest()]
    return '-'.join(L)
...
def parse_signed_cookie(cookie_str):
    try:
        ...
        if user is None:
            return None
        if md5 != hashlib.md5('%s-%s-%s-%s' % (id, passwd, expires, _COOKIE_KEY)).hexdugest():
            return None
        return user
    except:
        return None
...
@get('/signout')
def signout():
    ctx.request.delete_cookie(_COOKIE_NAME)
    raise seeother('/')
...
def authenticate():
    ...
    if user is None:
        ...
        raise APIError('auth:failed', 'passwd', 'Invaild passwd.')
    #make session cookie:
    max_age = 604800 if remember=='true' else None
    cookie = make_signed_cookie(user.id, user.passwd, max_age)
...
def register_user():
    ...
    if not passwd or not _RE_MD5.match(passwd):
        raise APIValueError('passwd')
    user = User.find_first('where email=?', email)
    if user:
        raise APIError('register:failed', 'email', 'Email is already in use.')
    user = User(name=name, email=email, passwd=passwd, image='http://www.gravatar.com/avatar/%s?d=mm&s=120' % hashlib.md5(email).hexdigest())
    user.insert()
    # make session cookie:
    cookie = make_signed_cookie(user.id, user.passwd, None)
...
def manage_blogs_create():
    return dict(id=None, action='/api/blogs', redirect='/manages/blogs', user=ctx.request.user)
